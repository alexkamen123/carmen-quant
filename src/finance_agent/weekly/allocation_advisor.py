...
语言要求：用大白话，像对完全不懂投资的朋友解释，避免金融术语缩写。

输出格式（严格 JSON，不要有其他内容）：
{
  "concentration_risk": "一句话说明当前最大集中风险，例如：钱都压在科技股了，一旦科技股跌就损失大",
  "macro_risk": "一句话说明当前宏观最大风险，用普通人能理解的语言",
  "hedge_directions": [
    {
      "direction": "方向名称（如：美债/防御股/黄金/新兴市场）",
      "rationale": "为什么买这个可以降低风险，一句大白话",
      "urgency": "高" | "中" | "低"
    }
  ]
}
...
语言要求：reason 字段必须用大白话，像对不懂投资的朋友解释，禁止用 RSI/EV/EBITDA 等缩写，
直接说「跌了多少」「盈利增速怎样」「跟现有持仓的关系是什么」。
...
type 判断标准：
- "防御/对冲型"：低贝塔、高股息、与科技股负相关或低相关（医疗、消费必需品、黄金、国债、公用事业）
- "成长型"：高成长、高估值、与科技股强相关（科技、互联网、半导体、AI）
- "ETF/宽基"：指数产品、大宗商品ETF、债券ETF
