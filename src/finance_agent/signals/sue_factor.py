...
def format_sue_note(ticker, asof=None, db_path=None) -> str:
    ...
    for r in rows:
        ...
        if sue is None or abs(sue) < thr:
            return ""                       # 最近一条也不过阈 → 不注入
        if sue <= -thr:
            return (f"【财报爆雷】{ticker} 最新财报每股收益(EPS)大幅低于分析师预期，"
                    f"低于幅度约为过去几年『实际比预期低多少』波动的 {abs(sue):.1f} 倍"
                    f"(SUE={sue:+.1f}σ，越负越反常)。基本面或已转弱、财报爆雷后股价常延续下行"
                    f"(盈余后漂移)，警惕别死扛，宜更保守/止损复核。")
        return (f"【财报大超预期】{ticker} 最新财报每股收益(EPS)大幅高于分析师预期，"
                f"超出幅度约为过去几年『实际比预期高多少』波动的 {abs(sue):.1f} 倍"
                f"(SUE={sue:+.1f}σ，越正越反常)。若已持有，减仓/止损前需额外确认、"
                f"避免因短期波动过早了结；此提示仅用于防卖飞，非增持信号。")
    return ""
